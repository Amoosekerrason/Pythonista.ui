...
class CalenderSectionView(View):
  ...
  def __init__(self,
    ...
    self.add_subview(CalenderHeaderContentView(self.width, self.height))
    self.add_subview(
...
class SelectMonthSectionView(View):
...
class InterfaceSectionView(View):

  def __init__(self, x=0, y=0):
    super().__init__()
    self.background_color = '#916aae'
    self.frame = (x, y, INTERFACE_WIDTH, INTERFACE_HEIGHT)
    self.add_subview(SelectMonthSectionView(parent=self))


class MainSectionView(View):
  ...
  def __init__(self):
    ...
    self.add_subview(
      CalenderSectionView(x=CALENDER_X, y=CALENDER_Y,
                   on_handle_date=self.handle_date))
    self.add_subview(InterfaceSectionView(x=INTERFACE_X, y=INTERFACE_Y))
